...
def get_unsupervised_embeddings(G, 
    ...
    return get_unsupervised_embedder(G, distance, seed, vector_length, use_relative_degrees, train_opts)(G.vs, G).cpu().detach().numpy()
...
def get_unsupervised_embedder(G, 
    ...
    mapper = StructuralMapper(G, distance, use_distances=True, num_workers=1, device=device, use_relative_degrees=use_relative_degrees)
